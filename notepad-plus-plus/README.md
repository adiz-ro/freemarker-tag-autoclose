...
Two things for `.ftl` and `.ftx` files, ported from the VS Code extension in this repo:

1. **Syntax highlighting** - `freemarker.udl.xml`, a **User Defined Language**: FreeMarker
   on top of HTML, with CSS and JavaScript in the same file. Once installed, `FreeMarker`
   shows up in the **Language** menu and `.ftl` / `.ftx` files are highlighted
   automatically when opened.
2. **Automatic tag closing** - `FreeMarkerAutoClose.py`, a script for the
   **PythonScript** plugin: typing `>` inserts the closing tag automatically, with the
   cursor left between the tags.

Requirements: Notepad++ 8.x (UDL 2.1 format) for highlighting, light theme, white
background. For automatic tag closing, also the **PythonScript** plugin (installable
from within Notepad++, see below).

## Installation - syntax highlighting (UDL)
...
## Installation - automatic tag closing (PythonScript)

A UDL cannot do this on its own (Notepad++ ties its native tag-closing feature strictly
to its native HTML/XML lexers, not to UDLs - see point 8 in the limitations section
below). The fix is `FreeMarkerAutoClose.py`, which ports
[`../src/freemarker.ts`](../src/freemarker.ts) (the same VS Code extension in this repo)
1:1 - the same block directives, the same content-less HTML elements, the same
`<#assign x>` (block) vs `<#assign x=1>` (standalone) rule. Verified against the same 83
cases from [`../test/freemarker.test.js`](../test/freemarker.test.js).
...
To uninstall: **Plugins -> Python Script -> Configuration...**, set **Initialisation**
back to **DISABLED**, delete the file from
...
8. **Automatic tag closing does not come from the UDL.** `Settings -> Preferences ->
   Auto-Completion -> Auto-Insert -> Close Tags` is the native feature that does this, but
   Notepad++ ties it strictly to its native HTML and XML lexers - not to UDLs, no matter
   how they are defined. That is why, before this UDL existed, when `.ftl`/`.ftx` were
   mapped to the native HTML lexer, auto-closing worked; once the extensions move to the
   `FreeMarker` UDL, the native feature stops applying. Plugins like **HTML Tag** do not
   help either - they do tag jumping/selection/renaming, not closing while typing, and
   rely on the same native feature. The fix that actually works is the
   `FreeMarkerAutoClose.py` script above, installed separately from the UDL.
